...
gb_internal lbValue lb_emit_load(lbProcedure *p, lbValue value) {
	...
	// NOTE: !invariant.load on constant globals would be beneficial, but Odin's
	// constants are either inlined at compile time or accessed through GEPs into
	// constant data (e.g. string literals), so LLVMIsAGlobalVariable never matches
	// here in practice. Revisit if codegen changes to produce direct global loads.
