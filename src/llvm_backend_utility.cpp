...
gb_internal lbValue lb_emit_min(lbProcedure *p, Type *t, lbValue x, lbValue y) {
	...
	Type *elem_type = is_type_simd_vector(t) ? base_array_type(t) : t;

	if (is_type_float(elem_type) && !is_arch_wasm()) {
	...
	if (is_type_integer(elem_type)) {
		char const *name = is_type_unsigned(elem_type) ? "llvm.umin" : "llvm.smin";
		LLVMValueRef args[2] = {x.value, y.value};
		LLVMTypeRef types[1] = {lb_type(p->module, t)};
		LLVMValueRef v = lb_call_intrinsic(p, name, args, gb_count_of(args), types, gb_count_of(types));
		return {v, t};
	}
...
gb_internal lbValue lb_emit_max(lbProcedure *p, Type *t, lbValue x, lbValue y) {
	...
	if (is_type_integer(elem_type)) {
		char const *name = is_type_unsigned(elem_type) ? "llvm.umax" : "llvm.smax";
		LLVMValueRef args[2] = {x.value, y.value};
		LLVMTypeRef types[1] = {lb_type(p->module, t)};
		LLVMValueRef v = lb_call_intrinsic(p, name, args, gb_count_of(args), types, gb_count_of(types));
		return {v, t};
...
gb_internal lbValue lb_emit_byte_swap(lbProcedure *p, lbValue value, Type *end_type) {
	...
	if (is_type_float(original_type)) {
		GB_ASSERT(!is_type_simd_vector(original_type));
