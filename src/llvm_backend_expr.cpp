...
gb_internal lbValue lb_emit_conv(lbProcedure *p, lbValue value, Type *t) {
	...
	if (is_type_complex(src) && is_type_complex(dst)) {
		...
		lbValue fields[2] = {real, imag};
		return lb_build_struct_value(p, t, fields, 2);
	}
	...
	if (is_type_quaternion(src) && is_type_quaternion(dst)) {
		...
		return lb_build_struct_value(p, t, fields, 4);
	}

	if ((is_type_integer(src) || is_type_float(src)) && is_type_complex(dst)) {
		Type *ft = base_complex_elem_type(dst);
		lbValue real = lb_emit_conv(p, value, ft);
		lbValue zero = {LLVMConstNull(lb_type(m, t)), t};
		return lb_emit_struct_iv(p, zero, real, 0);
	}


	if ((is_type_integer(src) || is_type_float(src)) && is_type_quaternion(dst)) {
		Type *ft = base_complex_elem_type(dst);
		lbValue real = lb_emit_conv(p, value, ft);
		// @QuaternionLayout — real (w) component is at index 3
		lbValue zero = {LLVMConstNull(lb_type(m, t)), t};
		return lb_emit_struct_iv(p, zero, real, 3);
	}
	if (is_type_complex(src) && is_type_quaternion(dst)) {
		Type *ft = base_complex_elem_type(dst);
		lbValue real = lb_emit_conv(p, lb_emit_struct_ev(p, value, 0), ft);
		lbValue imag = lb_emit_conv(p, lb_emit_struct_ev(p, value, 1), ft);
		// @QuaternionLayout — real (w) at index 3, imag (i) at index 0
		lbValue zero = {LLVMConstNull(lb_type(m, t)), t};
...
gb_internal lbValue lb_build_slice_expr_value(lbProcedure *p, Ast *expr) {
	...
	// Fused nested slice chain: src[a:b][c:d][e:f]...
	...
	// building a single aggregate at the end.
	{
		...
		if (inner_ast->kind == Ast_SliceExpr) {
			// Collect the chain: slices[0] is innermost, slices[n-1] is outermost (se).
			Ast *chain[8]; // bounded depth — 8 levels of nesting is more than enough
			isize chain_count = 0;
			chain[chain_count++] = expr; // outermost

			Ast *cursor = inner_ast;
			while (cursor->kind == Ast_SliceExpr && chain_count < 8) {
				chain[chain_count++] = cursor;
				cursor = unparen_expr(cursor->SliceExpr.expr);
			}
			// cursor is now the root source (not a SliceExpr).

			Type *root_type = base_type(type_of_expr(cursor));
			...
			if (is_type_slice(root_type) ||
				...
				lbValue src = lb_build_expr(p, cursor);
				...
				for (isize i = chain_count - 1; i >= 0; i--) {
					...
					lbValue hi = {};
					if (s->low  != nullptr) lo = lb_correct_endianness(p, lb_build_expr(p, s->low));
					if (s->high != nullptr) hi = lb_correct_endianness(p, lb_build_expr(p, s->high));
					if (hi.value == nullptr) hi = cur_len;

					bool no_idx = s->low == nullptr && s->high == nullptr;
					if (!no_idx) {
						lb_emit_slice_bounds_check(p, s->open, lo, hi, cur_len, s->low != nullptr);
					}

					cur_ptr = (s->low == nullptr)
						? cur_ptr
						: lb_emit_ptr_offset(p, cur_ptr, lo);
					cur_len = (s->low == nullptr)
						? lb_emit_conv(p, hi, t_int)
						: lb_emit_arith(p, Token_Sub, hi, lo, t_int);
				}
