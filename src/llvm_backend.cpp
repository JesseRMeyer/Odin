...
gb_internal bool lb_generate_code(lbGenerator *gen) {
	...
	if (build_context.lto_kind != LTO_None && build_context.metrics.os == TargetOs_windows) {
		auto paths = array_make<String>(permanent_allocator(), 0, 1);
		String path = concatenate_strings(permanent_allocator(), build_context.ODIN_ROOT, str_lit("\\bin\\llvm\\windows\\soft_float.obj"));
		array_add(&paths, path);
		Entity *lib = alloc_entity_library_name(nullptr, make_token_ident("soft_float"), nullptr, slice_from_array(paths), str_lit("soft_float"));
