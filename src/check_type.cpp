...
// (Type_Generic entities) reachable from the given scope. Used to detect
// return type expressions that depend on unresolved poly params.
	...
	for_array(i, results) {
		...
		if (field->type == nullptr) {
			...
			if (ctx->allow_polymorphic_types && ast_references_poly_params(ctx->scope, field->type)) {
				// Return type depends on unresolved poly params (e.g.
				// type_of(Q{}.data[0])). Use Type_Generic placeholder —
				// resolved during instantiation when params are concrete.
